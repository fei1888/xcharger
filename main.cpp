...
void can_write_handler() {
    ...
    /* Everyt 1sec charger send HB out */
    if (timer_tick%1000 == 0) {
        Can_tx.ID = 0x701;
        Can_tx.Data[0] = (uint8_t)charger_network_state;
        CAN_write(&Can_tx);
        DEBUG_LOG("XNERGY_INFO: charge sent a heartbeat msg out\n");
    }
    ...
    switch (charger_network_state) {
        ...
        case XNERGY_NETWORK_OPERATE:
            ...
            if (timer_tick%200 == 0) {
                ...
                CAN_write(&Can_tx);
                DEBUG_LOG("XNERGY_INFO: charge sent a msg to BMS\n");
            }
            break;
        default:
            break;
    ...
    /* reset the counter everysec, so that not overflow */
    if (timer_tick >= 1000) {
        timer_tick = 0;
    }
...
void can_read_handler(void) {
    ...
    if (CAN_read(&Can_rx)==true) {
        switch (Can_rx.ID) {
            case XNERGY_CAN_CTRL:
                ...
                DEBUG_LOG("XNERGY_INFO: charge received a msg from BMS\n");
                timeout = 0;
                break;
            case XNERGY_CAN_HB: 
    ...
    /* no BMS msg recevied after 5sec, switch the state back to pre-op and idle */
    if (timeout >= 5000) {
        charger_control.enable_command = false;
        charger_operate_state = XNERGY_CHARGER_IDLE_STAT;
        charger_network_state = XNERGY_NETWORK_PRE_OPERATE;
